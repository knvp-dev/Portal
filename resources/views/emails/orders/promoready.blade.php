...
<body>
	...
	<div class="container">
		...
		Uw bestelling voor promomateriaal met nummer {{ $order->id }} staat klaar voor verzending. <br><br>
		@if(!$order->complete)
		<br><br>
		<span class="red">Let op! De inhoud van deze levering wijkt af, er zijn producten niet leverbaar.</span>
		@endif
		<br><br>
		Voor meer informatie over deze bestelling, <a href="http://portal.dev/promomateriaal/detail/{{$order->id}}">klik hier</a>.
		<br><br>
		<hr>
		<br><br>
		Chèr Collègues de Konvert {{ $order->user->name }}, <br><br>
		Votre commande de matériel de promotion avec numéro de commande {{ $order->id }} est prête à être livrée.<br><br>
		@if(!$order->complete)
		<br><br>
		...
		@endif
		<br><br>
		Pour plus d’informations, <a href="http://portal.dev/promomateriaal/detail/{{$order->id}}">cliquez ici</a>.
		<br><br>
